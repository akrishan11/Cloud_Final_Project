...
- [Live Site](https://d1vrs4hix1vyxh.cloudfront.net)
- [Slides](https://docs.google.com/presentation/d/1JhoBstbPhS3i7MwVkMsvE7GQdVNB1bqFRj1HIeuMLro/edit?usp=sharing)
- [Demo Video](https://youtu.be/yV5t_gOwy3o)
...
| #   | Service                                           | Purpose                                                                                                             | Why we picked it                                                                                                                                                                                                      | Presenter |
| --- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| 1   | **Amazon Cognito** (User Pool + PostConfirmation) | Email/password sign-up, login, JWT issuance, $1000 balance provisioning                                             | We didn't want to build our own auth system from scratch. Cognito handles passwords, tokens, and email verification for free essentially. The PostConfirmation trigger was a clean way to give every new user their starting balance automatically. | Shreyash  |
| 2   | **API Gateway HTTP API**                          | REST routes (`/markets`, `/me`, `/leaderboard`, `/markets/{id}/bets`, `/markets/{id}/resolve`)                      | The HTTP API type has JWT authorization built in — you just point it at Cognito and it validates tokens without needing a custom Lambda. It's also cheaper than the older REST API and was fast to set up.            | Shreyash  |
| 3   | **API Gateway WebSocket API**                     | Real-time price updates pushed to subscribed clients                                                                | We needed actual push from server to client so polling wasn't going to cut it. API Gateway manages all the connection state so we don't have to run a persistent server just to hold WebSocket connections open.      | Akash     |
| 4   | **AWS Lambda** (15 functions, ARM64 Node.js 20)   | All business logic: market CRUD, bet placement, payout fan-out, WebSocket handlers, scheduler callback, leaderboard | Betting traffic is totally unpredictable — it spikes right when class starts and goes quiet otherwise. Lambda meant we weren't paying for idle compute, and we never had to worry about scaling.                       | Akash     |
| 5   | **Amazon DynamoDB** (4 tables)                    | Primary data store: users, markets, positions, websocket connections                                                | We needed atomic writes across multiple fields (balance + market volume + position) in a single operation. DynamoDB's `TransactWriteItems` handles that cleanly. Also no schema to manage, which made iteration fast.  | Aidan     |
| 6   | **DynamoDB Streams** (MarketsTable)               | Event source for WebSocket price broadcast                                                                          | Every time a bet changes the market price in DynamoDB, Streams fires an event automatically. We wired that to a Lambda that pushes the update to connected clients. It keeps the bet path and the broadcast path separate so a slow client can't block a bet. | Aidan     |
| 7   | **Amazon EventBridge Scheduler**                  | Per-market one-time schedules for `scheduled → open` and `open → closed` transitions                                | We needed markets to open and close at specific times without anyone having to manually do it. EventBridge Scheduler lets you create a one-time "fire at exactly 6pm" schedule per market, and it deletes itself after running. Way cleaner than a cron job polling DynamoDB. | Krishna   |
| 8   | **Amazon S3** (private, OAC)                      | Host Vite-built frontend static assets                                                                              | It's the simplest and cheapest way to host static files. We kept the bucket private and only let CloudFront read from it — no public access, no accidental exposure.                                                  | Krishna   |
| 9   | **Amazon CloudFront** (+ OAC)                     | HTTPS edge delivery of the SPA; SPA routing fallback                                                                | HTTPS is required for Cognito to work in the browser, and CloudFront gives you that plus a CDN edge for free essentially. We also ran into an issue where S3 was returning 403 instead of 404 for unknown paths, breaking React Router — CloudFront's custom error responses fixed that. | Haiden    |
